...
export class AssociatesController {
  ...
  async create(
    ...
    //Sub ordinates roles
    subOrdinate.role_id = ROLES.ASSOCIATES;

    return this.adminRepository.create(subOrdinate);
  }

  @get('/cp/v1/associates')
  @response(200, {
  ...
  async find(): Promise<Admin[]> {
    return this.adminRepository.find({
      where: {role_id: ROLES.ASSOCIATES},
      fields: {
        password: false,
        created_at: false,
        updated_at: false,
